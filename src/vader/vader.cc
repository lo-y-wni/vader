...
void Vader::changeVar(atlas::FieldSet * afieldset,
   ...
   oops::Log::debug() << "neededVars passed to Vader::changeVar: " << neededVars
      << std::endl;
   ...
   for (auto targetVariable : targetVariables) {
      oops::Log::debug() << "Vader::changeVar calling Vader::getVariable for: "
...
bool Vader::getVariable(atlas::FieldSet * afieldset,
   ...
   if ((recipeList != cookbook_.end()) && !recipeList->second.empty()) {
      oops::Log::debug() << "Vader cookbook contains at least one recipe for '"
         << targetVariable << "'" << std::endl;
      for (int i=0; i < recipeList->second.size(); ++i) {
         ...
         for (auto ingredient : recipeList->second[i]->ingredients()) {
            haveIngredient = 
               (std::find(fieldSetFieldNames.begin(), fieldSetFieldNames.end(),
         ...
         if (haveIngredient) {
            ...
            oops::Log::debug() <<
               "Do not have all the ingredients for this recipe." << std::endl;
         }
      }
   }
   else {
