...
bool evalTotalMassMoistAir(atlas::FieldSet & stateFlds)
{
...
bool evalRatioToMt(atlas::FieldSet & stateFlds, const std::vector<std::string> & vars)
{
  oops::Log::trace() << "[evalRatioToMt()] starting ..." << std::endl;

  // stateFlds[0] = m_x = [ mv | mci | mcl | m_r ]
  const auto ds_m_x  = make_view<const double, 2>(stateFlds[vars[0]]);
  const auto ds_m_t  = make_view<const double, 2>(stateFlds[vars[1]]);
  auto ds_tfield  = make_view<double, 2>(stateFlds[vars[2]]);

  auto fspace = stateFlds[vars[2]].functionspace();

  auto evaluateRatioToMt = [&] (idx_t i, idx_t j) {
    ds_tfield(i, j) = ds_m_x(i, j) / ds_m_t(i, j);
  };

  auto conf = Config("levels", stateFlds[vars[2]].shape(1)) |
              Config("include_halo", false);

  // This code uses a FunctionSpace parallel_for with config option "include_halo" to avoid risky
  // halo operations in the lambda evaluateRatioToMt above. But this config is only supported by
  // the CubedSphereColumns FunctionSpace. Therefore, check the FunctionSpace...
  ASSERT_MSG(fspace.type() == "CubedSphereColumns",
             "evalRatioToMt got invalid FunctionSpace type = " + fspace.type());

  functions::parallelFor(fspace, evaluateRatioToMt, conf);

  stateFlds[vars[2]].set_dirty();
...
bool evalTotalRelativeHumidity(atlas::FieldSet & stateFlds)
{
  ...
  const auto qrainView = make_view<const double, 2>(stateFlds["qrain"]);
  const auto qsatView = make_view<const double, 2>(stateFlds["qsat"]);
  auto rhtView = make_view<double, 2>(stateFlds["rht"]);

  auto conf = Config("levels", stateFlds["rht"].shape(1)) |
              Config("include_halo", false);
  ...
  auto fspace = stateFlds["rht"].functionspace();

  // This code uses a FunctionSpace parallel_for with config option "include_halo" to avoid risky
  // halo operations in the lambda evaluateRatioToMt above. But this config is only supported by
  // the CubedSphereColumns FunctionSpace. Therefore, check the FunctionSpace...
  ASSERT_MSG(fspace.type() == "CubedSphereColumns",
             "evalTotalRelativeHumidity got invalid FunctionSpace type = " + fspace.type());

  functions::parallelFor(fspace, evaluateRHT, conf);

  stateFlds["rht"].set_dirty();
...
bool evalMassCloudIce(atlas::FieldSet & stateFlds)
{
...
bool evalMassCloudLiquid(atlas::FieldSet & stateFlds)
{
...
bool evalMassRain(atlas::FieldSet & stateFlds)
{
...
bool evalAirTemperature(atlas::FieldSet & stateFlds)
{
  ...
  auto conf = Config("levels", stateFlds["air_temperature"].shape(1)) |
              Config("include_halo", false);
...
bool evalSpecificHumidityFromRH_2m(atlas::FieldSet & stateFlds)
{
  ...
  auto fspace = stateFlds["specific_humidity_at_two_meters_above_surface"].functionspace();

  auto evaluateSpecificHumidity_2m = [&] (idx_t i, idx_t j) {
    ds_q2m(i, j) = 0.01 * ds_rh(i, j) * ds_qsat(i, j); };

  auto conf = Config("levels",
    stateFlds["specific_humidity_at_two_meters_above_surface"].shape(1)) |
              Config("include_halo", false);
...
bool evalGpHeightMinusOne(atlas::FieldSet & stateFlds)
{
  oops::Log::trace() << "[evalGpHeightMinusOne()] starting ..." << std::endl;

  const auto height_lmoView = make_view<const double, 2>(stateFlds["height_levels_minus_one"]);
  auto gph_lmoView = make_view<double, 2>(stateFlds["geopotential_height_levels_minus_one"]);

  gph_lmoView.assign(height_lmoView);
  stateFlds["geopotential_height_levels_minus_one"].set_dirty();
